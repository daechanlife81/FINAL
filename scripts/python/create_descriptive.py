...
subtitle_font = Font(name="Times New Roman", bold=True, size=11)
...
def write_row(ws, r, vals, is_section=False):
...
def write_freq(ws, r, var_name, col, categories):
    ...
    if r - start_r > 1:
        ...
        ws.cell(row=start_r, column=1).alignment = Alignment(horizontal="left", vertical="center")
    return r
...
# 조사지역
regions = [("서울", 1), ("부산", 2), ("대구", 3), ("인천", 4), ("광주", 5),
           ("대전", 6), ("울산", 7), ("세종", 8), ("경기", 9), ("강원", 10),
           ("충북", 11), ("충남", 12), ("전북", 13), ("전남", 14), ("경북", 15),
           ("경남", 16), ("제주", 17)]
row = write_freq(ws1, row, "조사지역", "SQ3", regions)
...
# ========== Sheet 2: 측정변수 기술통계 ==========
ws2 = wb.create_sheet("측정변수 기술통계")

ws2.merge_cells("A1:H1")
ws2["A1"] = "측정변수의 기술통계 및 정규성 검토 (N=1,328)"
ws2["A1"].font = title_font
ws2["A1"].alignment = center

headers2 = ["구분", "변수", "문항", "M", "SD", "Min", "Max", "왜도", "첨도"]
...
variables = [
    # 통제변수
    ("통제변수", "성별", "0=남, 1=여", "re_SQ1"),
    ("", "연령", "만 나이", "SQ2"),
    ("", "연령대", "1=19~29세, 2=30대, 3=40대, 4=50대, 5=60~79세", "HSQ2"),
    ("", "혼인상태", "1~5", "D_04"),
    ("", "과거 자원봉사경험", "0=없음, 1=있음", "re_B_02"),
    # 인간자본
    ("독립변수\n(인간자본)", "주관적 건강", "1~5", "D_05"),
    ("", "교육수준", "1~5", "D_03"),
    # 사회자본
    ("독립변수\n(사회자본)", "일반적 신뢰", "1~4", "C_05"),
    ("", "네트워크", "0=부족, 1=충분", "re_C_06"),
    ("", "사회단체활동", "0=미참여, 1=참여", "re_C_07"),
    # 문화자본
    ("독립변수\n(문화자본)", "종교활동", "0=미참여, 1=참여", "re_C_08"),
    ("", "자원봉사제도 인식1", "관리시스템", "B_06_1"),
    ("", "자원봉사제도 인식2", "종합보험", "B_06_2"),
    ("", "자원봉사제도 인식3", "자원봉사센터", "B_06_3"),
    ("", "자원봉사제도 인식4", "마일리지/쿠폰", "B_06_4"),
    ("", "자원봉사제도 인식5", "기부금처리 제도", "B_06_5"),
    ("", "자원봉사제도 인식6", "자원봉사활동기본법", "B_06_6"),
    ("", "자원봉사제도 인식7", "자원봉사자의 날", "B_06_7"),
    ("", "자원봉사제도 인식8", "봉사 대상", "B_06_8"),
    ("", "자원봉사교육 경험", "0=없음, 1=있음", "re_B_04"),
    # 매개변수 - 효능감
    ("매개변수\n(효능감)", "효능감1", "의미있는 기여", "B_05_1"),
    ("", "효능감2", "기회 탐색", "B_05_2"),
    ("", "효능감3", "사회정의 실현", "B_05_3"),
    ("", "효능감4", "지역사회 변화", "B_05_4"),
    ("", "효능감5", "도움 제공", "B_05_5"),
    ...
    ("", "삶의 만족", "0~10", "C_04"),
    # 종속변수
    ("종속변수", "자원봉사 참여경험", "0=없다, 1=있다", "re_A_01_1"),
]
...
for group, var_name, item, col in variables:
    m = df[col].mean()
    sd = df[col].std()
    mn = df[col].min()
    mx = df[col].max()
    ...
    if group and group != prev_group:
        if prev_group and group_start and group_start < r - 1:
        ...
        elif prev_group and group_start:
            ws2.cell(row=group_start, column=1).alignment = center_wrap
        group_start = r
        prev_group = group

    vals = [group if group else "", var_name, item,
            f"{m:.2f}", f"{sd:.2f}", f"{mn}", f"{mx}", f"{skew:.3f}", f"{kurt:.3f}"]
    for c, v in enumerate(vals, 1):
...
# 마지막 그룹 병합
if group_start and group_start < r - 1:
    ws2.merge_cells(start_row=group_start, start_column=1, end_row=r-1, end_column=1)
    ws2.cell(row=group_start, column=1).alignment = center_wrap
...
ws2[f"A{r}"] = "주. 정규성 판단: |왜도|<3, |첨도|<10 (Kline, 2015). 모든 분석 변수가 허용 범위 내."
ws2[f"A{r}"].font = note_font
...
wb.save("descriptive_statistics.xlsx")
print("저장 완료: descriptive_statistics.xlsx")

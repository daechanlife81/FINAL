...
# 잠재변수 평균 산출
df["efficacy_mean"] = df[[f"B_05_{i}" for i in range(1,11)]].mean(axis=1)
df["awareness_mean"] = df[[f"B_06_{i}" for i in range(1,9)]].mean(axis=1)
df["wellbeing_mean"] = df[["C_01","re_C_02","re_C_03","C_04"]].mean(axis=1)
...
def write_freq(ws, r, var_name, col, categories):
    ...
    for label, codes in categories:
        count = df[col].isin(codes).sum() if isinstance(codes, list) else (df[col] == codes).sum()
...
row = write_freq(ws1, row, "연령대", "HSQ2", [("만 19~29세", 1), ("30대", 2), ("40대", 3), ("50대", 4), ("60~79세", 5)])
row = write_freq(ws1, row, "조사지역", "SQ3", [("서울",1),("부산",2),("대구",3),("인천",4),("광주",5),("대전",6),("울산",7),("세종",8),("경기",9),("강원",10),("충북",11),("충남",12),("전북",13),("전남",14),("경북",15),("경남",16),("제주",17)])
row = write_freq(ws1, row, "교육수준", "D_03", [("초등학교 이하",1),("중학교",2),("고등학교",3),("대학교",4),("대학원 이상",5)])
row = write_freq(ws1, row, "혼인상태", "D_04", [("미혼",1),("배우자 있음",2),("사별",3),("이혼",4),("별거",5)])
row = write_freq(ws1, row, "월평균\n가구소득", "D_08", [("100만원 미만",1),("100~200만원 미만",2),("200~300만원 미만",3),("300~400만원 미만",4),("400~500만원 미만",5),("500~600만원 미만",6),("600~700만원 미만",7),("700~800만원 미만",8),("800만원 이상",9)])
...
for i, h in enumerate(["구분", "변수", "범주", "빈도(명)", "비율(%)"], 1):
...
for group, var_name, col, label_0, label_1 in binary_vars:
    ...
    for c, v in enumerate([group if group else "", var_name, label_0, count_0, f"{pct_0:.1f}"], 1):
    ...
    for c, v in enumerate(["", "", label_1, count_1, f"{pct_1:.1f}"], 1):
        cell = ws2.cell(row=r, column=c, value=v)
        cell.font = body_font
        cell.alignment = center if c >= 4 else left_align
        cell.border = border_mid
...
for i, h in enumerate(["구분", "변수", "문항 내용", "M", "SD", "왜도", "첨도", "α"], 1):
    cell = ws3.cell(row=3, column=i, value=h)
    cell.font = header_font
    cell.alignment = center
    cell.border = border_top

# is_total: True이면 볼드 처리
continuous_vars = [
    ("통제변수", "연령", "", "SQ2", None, False),
    ("", "혼인상태", "", "D_04", None, False),
    ("인간자본", "주관적 건강", "", "D_05", None, False),
    ("", "교육수준", "", "D_03", None, False),
    ("사회자본", "일반적 신뢰", "", "C_05", None, False),
    ("문화자본", "제도인식(전체)", "8문항 평균", "awareness_mean", ".910", True),
    ("", "  제도인식1", "관리시스템", "B_06_1", None, False),
    ("", "  제도인식2", "종합보험", "B_06_2", None, False),
    ("", "  제도인식3", "자원봉사센터", "B_06_3", None, False),
    ("", "  제도인식4", "마일리지/쿠폰", "B_06_4", None, False),
    ("", "  제도인식5", "기부금처리 제도", "B_06_5", None, False),
    ("", "  제도인식6", "자원봉사활동기본법", "B_06_6", None, False),
    ("", "  제도인식7", "자원봉사자의 날", "B_06_7", None, False),
    ("", "  제도인식8", "대한민국 자원봉사 대상", "B_06_8", None, False),
    ("매개변수\n(효능감)", "효능감(전체)", "10문항 평균", "efficacy_mean", ".889", True),
    ("", "  효능감1", "의미있는 기여", "B_05_1", None, False),
    ("", "  효능감2", "기회 탐색", "B_05_2", None, False),
    ("", "  효능감3", "사회정의 실현", "B_05_3", None, False),
    ("", "  효능감4", "지역사회 변화", "B_05_4", None, False),
    ("", "  효능감5", "도움 제공", "B_05_5", None, False),
    ("", "  효능감6", "전문가 교류", "B_05_6", None, False),
    ("", "  효능감7", "동등 기회", "B_05_7", None, False),
    ("", "  효능감8", "지식 적용", "B_05_8", None, False),
    ("", "  효능감9", "자기돌봄 지원", "B_05_9", None, False),
    ("", "  효능감10", "참여 의향", "B_05_10", None, False),
    ("매개변수\n(안녕감)", "안녕감(전체)", "4문항 평균", "wellbeing_mean", ".696", True),
    ("", "  행복", "", "C_01", None, False),
    ("", "  걱정(역코딩)", "", "re_C_02", None, False),
    ("", "  우울(역코딩)", "", "re_C_03", None, False),
    ("", "  삶의 만족", "", "C_04", None, False),
]
...
for group, var_name, item, col, alpha, is_total in continuous_vars:
    m = df[col].mean()
    sd = df[col].std()
    skew = df[col].skew()
    ...
    use_font = bold_font if is_total else body_font
...
ws3[f"A{r}"] = "주. 정규성 기준: |왜도|<3, |첨도|<10 (Kline, 2015). 모든 변수 허용 범위 내."
ws3[f"A{r}"].font = note_font
r += 1
ws3.merge_cells(f"A{r}:H{r}")
ws3[f"A{r}"] = "    α=Cronbach's α. 전체 행은 하위문항 평균값의 기술통계."
ws3[f"A{r}"].font = note_font
...
print("saved: descriptive_statistics_v2.xlsx")

## ============================================================
## 방안 3: 양적/음적 안녕감 분리 모형 (2개 잠재변수)
## 박사학위논문 마지막 점검 - 학술적 최우수 옵션
##
## 변경사항:
##   - 양적 안녕감 (pos_wb) = C_01(만족) + C_04(행복)
##   - 음적 안녕감 (neg_wb) = re_C_02(걱정역) + re_C_03(우울역)
## ============================================================
...
for (lv in c("efficacy", "awareness", "pos_wb", "neg_wb")) {
...
## 양적/음적 안녕감 상관
cor_wb <- std_load[std_load$op == "~~" &
                    ((std_load$lhs == "pos_wb" & std_load$rhs == "neg_wb") |
                     (std_load$lhs == "neg_wb" & std_load$rhs == "pos_wb")), ]
if (nrow(cor_wb) > 0) {
  cat(sprintf("\n양적-음적 안녕감 상관: r = %.3f\n", cor_wb$est.std[1]))
}
...
sem_model <- '
  ...
  pos_wb ~ D_05 + D_03 + C_05 + re_C_06 + re_C_07 + re_C_08
  neg_wb ~ D_05 + D_03 + C_05 + re_C_06 + re_C_07 + re_C_08
  ...
  pos_wb ~ re_SQ1 + SQ2 + D_04 + re_B_02
  neg_wb ~ re_SQ1 + SQ2 + D_04 + re_B_02
...
path_labels <- list(
  "efficacy" = "효능감", "pos_wb" = "양적안녕", "neg_wb" = "음적안녕",
...
for (dv in c("efficacy", "pos_wb", "neg_wb", "re_A_01_1")) {
  cat(sprintf("\n[%s 대상 경로]\n", get_label(dv)))
  iv_list <- if (dv %in% c("pos_wb", "neg_wb")) iv_wb else main_iv
...
cat("\n--- 매개변수 경로 ---\n")
med_paths <- list(
  c("pos_wb","efficacy"), c("neg_wb","efficacy"),
  c("re_A_01_1","efficacy"), c("re_A_01_1","pos_wb"), c("re_A_01_1","neg_wb")
)
for (p in med_paths) {
...
cat(sprintf("양적안녕: R2 = %.3f\n", r2["pos_wb"]))
cat(sprintf("음적안녕: R2 = %.3f\n", r2["neg_wb"]))
...
## ============================================================
cat("\n================================================================\n")
cat("[방안 3] 매개효과 (ML + Bootstrap 5,000회)\n")
cat("================================================================\n")
...
sem_mediation <- '
  ...
  pos_wb ~ c1*D_05 + c2*D_03 + c3*C_05 + c4*re_C_06 + c5*re_C_07 + c6*re_C_08
  neg_wb ~ e1*D_05 + e2*D_03 + e3*C_05 + e4*re_C_06 + e5*re_C_07 + e6*re_C_08
  re_A_01_1 ~ d1*D_05 + d2*D_03 + d3*C_05 + d4*re_C_06 + d5*re_C_07 + d6*awareness + d7*re_C_08 + d8*re_B_04
  pos_wb ~ b1*efficacy
  neg_wb ~ b2*efficacy
  re_A_01_1 ~ b3*efficacy
  re_A_01_1 ~ b4*pos_wb
  re_A_01_1 ~ b5*neg_wb
  ...
  # 단순매개 (효능감)
  ind_eff1 := a1*b3; ind_eff2 := a2*b3; ind_eff3 := a3*b3; ind_eff4 := a4*b3
  ind_eff5 := a5*b3; ind_eff6 := a6*b3; ind_eff7 := a7*b3; ind_eff8 := a8*b3

  # 단순매개 (양적안녕)
  ind_pos1 := c1*b4; ind_pos2 := c2*b4; ind_pos3 := c3*b4
  ind_pos4 := c4*b4; ind_pos5 := c5*b4; ind_pos6 := c6*b4

  # 단순매개 (음적안녕)
  ind_neg1 := e1*b5; ind_neg2 := e2*b5; ind_neg3 := e3*b5
  ind_neg4 := e4*b5; ind_neg5 := e5*b5; ind_neg6 := e6*b5
'
...
cat("\n--- 단순매개: 양적안녕 ---\n")
for (i in 1:6) {
  row <- defined[defined$lhs == paste0("ind_pos", i), ]
  sig <- ifelse(row$ci.lower > 0 | row$ci.upper < 0, "유의", "비유의")
  cat(sprintf("  %s: beta=%.4f, 95%%CI[%.4f, %.4f] %s\n",
...
cat("\n--- 단순매개: 음적안녕 ---\n")
for (i in 1:6) {
  row <- defined[defined$lhs == paste0("ind_neg", i), ]
  sig <- ifelse(row$ci.lower > 0 | row$ci.upper < 0, "유의", "비유의")
  cat(sprintf("  %s: beta=%.4f, 95%%CI[%.4f, %.4f] %s\n",
    iv_wb_labels[i], row$std.all, row$ci.lower, row$ci.upper, sig))
}
